...
function woocommerce_product_addons_init() {
	...
	if ( ! function_exists( 'phpversion' ) || version_compare( phpversion(), '7.4.0', '<' ) ) {
		...
		$notice = sprintf(
			__(
				'Woo Product Add-Ons requires at least PHP <strong>%1$s</strong>. Learn <a href="%2$s">how to update PHP</a>.',
	...
	if ( ! function_exists( 'WC' ) || version_compare( WC()->version, '8.2.0' ) < 0 ) {
		$notice = __( 'Woo Product Add-Ons requires at least WooCommerce version <strong>%1$s</strong>. %2$s', 'woocommerce-product-addons' );
	...
	if ( ! class_exists( 'WC_Product_Addons' ) ) :
		...
		define( 'WC_PRODUCT_ADDONS_VERSION', '7.0.1' ); // WRCS: DEFINED_VERSION.
