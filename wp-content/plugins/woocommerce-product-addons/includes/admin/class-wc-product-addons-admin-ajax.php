...
class WC_Product_Addons_Admin_Ajax {
	...
	public static function ajax_edit_addon_order_item() {
		...
		list( $order, $item, $product ) = self::validate_request_and_fetch_data( $failure );
		...
		$valid      = $addon_cart->validate_add_cart_item( true, $item[ 'product_id' ], 1, $_POST );
		/**
		...
		$values = $addon_cart->add_cart_item_data( array(), $item[ 'product_id' ] );
	 ...
	 * @return array
	 */
	public static function validate_request_and_fetch_data( array $failure ): array {
